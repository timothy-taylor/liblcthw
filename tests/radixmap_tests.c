...
static char *test_operations()
{
    ...
    mu_assert(make_random(map), "Failed to make a random radix map");

    RadixMap_sort(map);
    mu_assert(check_order(map), "Failed to sort map");

    mu_assert(test_search(map), "Failed the search test");
    mu_assert(check_order(map), "Failed to stay sorted after search");
    ...
    while (map->end > 0){
        ...
        mu_assert(check_order(map), "Failed to stay sorted after delete");
    }
