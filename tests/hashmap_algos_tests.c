...
enum { ALGO_FNV1A, ALGO_ALDER32, ALGO_DJB };
...
char *test_distribution()
{
    ...
    fill_distribution(stats[ALGO_ALDER32], keys, Hashmap_adler32_hash);
    ...
    for (i = 0; i < BUCKETS; i++){
        fprintf(stderr, "%d\t%d\t%d\n",
                stats[ALGO_FNV1A][i],
                stats[ALGO_ALDER32][i], stats[ALGO_DJB][i]);
    }
